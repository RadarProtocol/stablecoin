...
fn.tags = ['Core', 'Swapper', 'AvaxSwapper'];
fn.dependencies = ['Config', 'LickHitter', 'USDR'];
fn.skip = async (hre) => {
  // Skip this on non-core deployments or when network isn't avalanche
  const config = await loadConfig(hre);
  return config.DEPLOYMENT_TYPE != "Swappers" || (config.NETWORK !== 43114 && !config.isDevDeploy ) || !config.SWAPPERS.swappersToDeploy.includes("AvaxSwapper");
};
