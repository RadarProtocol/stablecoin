...
import { deployUSDR3PoolCurveFactoryAvalanche, setavaxav3CRVTokenBalance, setavaxWAVAXTokenBalance, setavaxSAVAXTokenBalance } from '../utils/USDRCurve';
...
const snapshot = async () => {
    ...
    const SAVAX = stableFactory.attach("0x2b2C81e08f1Af8835a78Bb2A90AE924ACE0eA4bE");
    ...
    await yieldVault.addSupportedToken(SAVAX.address, BUFFER);
    ...
    const swapperFactory = await ethers.getContractFactory("BenqiStakedAvaxSwapper");
...
const checkSwapperEmptyBalance = async (
    ...
    const b2 = await SAVAX.balanceOf(swapper.address);
...
describe('Avalanche: AvaxSwapper', () => {
    it("depositHook", async () => {
        ...
        const minsAVAX = minUSDT.sub(minUSDT.mul(SLIPPAGE_TOLERANCE).div(10000)).mul(10**12).div(avaxPrice);
        const myMinCollateral = minsAVAX.add(directDeposit);
        ...
        // Direct deposit 
        await setavaxSAVAXTokenBalance(deployer, directDeposit);
        await SAVAX.connect(deployer).transfer(swapper.address, directDeposit);

        // Borrow
        await USDR.mint(swapper.address, borrow);

        // Do swap
        await swapper.depositHook(SAVAX.address, swapData);

        const mySharesBal = await yieldVault.balanceOf(SAVAX.address, deployer.address);
        const myBal = await yieldVault.convertShares(SAVAX.address, mySharesBal, 0);
        expect(myBal).to.be.gte(myMinCollateral);

        await checkSwapperEmptyBalance(swapper, USDR, SAVAX, av3Crv);
    });
    it("repayHook", async () => {
        const {
            swapper,
        ...
        // Remove collateral
        await setavaxSAVAXTokenBalance(deployer, collatRemoved);
        await SAVAX.transfer(swapper.address, collatRemoved);

        // Do swap
        await swapper.repayHook(SAVAX.address, swapData);
    ...
    it("liquidateHook", async () => {
        ...
        // Remove collateral
        await setavaxSAVAXTokenBalance(deployer, collateralLiquidated);
        await SAVAX.transfer(swapper.address, collateralLiquidated);
        ...
        await checkSwapperEmptyBalance(swapper, USDR, av3Crv, SAVAX);
    });
});